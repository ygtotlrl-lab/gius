...
const APP = {
  ...
  appClasses: {
    'auth-err':
      'הודעת שגיאת הכניסה בגיוס היא תיבה צבועה שנפתחת רק כשיש שגיאה — מסך הכניסה כאן קצר, ושורה ריקה ששמורה מראש נראית בו כשדה חסר — ⚠️ מוצהרת **עד יישור שכבת האסימונים**',
    'back':
      'כפתור החזרה בגיוס הוא קישור בגוון הזהות מעל הכותרת — המסכים כאן נערמים לעומק, והחזרה היא צעד אחורה ולא פעולה — ⚠️ מוצהרת **עד יישור שכבת האסימונים**',
    'badge':
      'תג בגיוס נושא רקע ודיו כברירת מחדל — הוא מציג מצב התחייבות גם כשאין לו סיווג, ובלעדיהם הוא טקסט חשוף בשורה — ⚠️ מוצהרת **עד יישור שכבת האסימונים**',
    'btn':
      'כפתור בגיוס נושא רדיוס קטן ומצב לחיצה שוקע — הוא יושב בתוך טפסים צפופים של סכומים, ולא בשורת פעולה עליונה — ⚠️ מוצהרת **עד יישור שכבת האסימונים**',
    'card':
      'כרטיס בגיוס נושא ריווח תחתון גדול — המסך כאן הוא ערימת כרטיסים נגללת, והרווח ביניהם הוא מה שמפריד תורם מתורם — ⚠️ מוצהרת **עד יישור שכבת האסימונים**',
    'fab':
      'כפתור הפעולה הצף בגיוס יושב מעל הסרגל התחתון בקצה המסיים — הניווט כאן תחתון, וכפתור שיושב עליו היה מכסה לשונית — ⚠️ מוצהרת **עד יישור שכבת האסימונים**',
    'fld':
      'שדה בגיוס הוא בלוק עם ריווח תחתון — הטפסים כאן ארוכים, והתווית יושבת בתוך השדה ולא מעליו — ⚠️ מוצהרת **עד יישור שכבת האסימונים**',
    'hero':
      'הכותרת בגיוס היא פריסת טור ממורכזת של כרטיס היעד — היא מציגה מונה ואחוז, ואינה משטח צבוע ברוחב המסך — ⚠️ מוצהרת **עד יישור שכבת האסימונים**',
    'sheet':
      'הגיליון התחתון בגיוס נכנס בהנפשה ונושא ריווח לשולי המכשיר — הוא נפתח מעל סרגל תחתון, ובלעדיו הכפתור התחתון נחתך — ⚠️ מוצהרת **עד יישור שכבת האסימונים**',
    'sheet-bd':
      'גוף הגיליון בגיוס הוא טופס ולא פסקה — ולכן אין בו גודל גופן וגובה שורה משלו, והשדות קובעים — ⚠️ מוצהרת **עד יישור שכבת האסימונים**',
    'sheet-hd':
      'כותרת הגיליון בגיוס נושאת את רדיוס הפינות העליונות — הגיליון נדבק לתחתית המסך, והכותרת היא הקצה העליון שלו — ⚠️ מוצהרת **עד יישור שכבת האסימונים**',
    'stat':
      'תא המונה בגיוס הוא כרטיס עצמאי עם מסגרת ורקע — הוא יושב ברשת של ארבעה, ולא בשורה מחולקת בקו — ⚠️ מוצהרת **עד יישור שכבת האסימונים**',
  },
};
/* ── סוף APP ───────────────────────────────────────────────────────────── */
