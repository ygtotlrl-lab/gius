...
const APP = {
  ...
  gapRows: [41, 74, 112, 114, 118, 139, 147, 59],
  ...
  swReload: { mode: 'user', why: '`skipWaiting` כבוי כאן בהחלטת מנהל — ⛔ העובד החדש ממתין עד שנלחץ הבאנר, ⚠️ ו-`controllerchange` אינו יורה על עדכון שלא התבקש' },
  ...
  tableProbe: {
     ...
     *  ניתנת למדידה במלואה. */
    158: (c) => c.hasCode(/G_PASS_ITER\s*=\s*100000\b/)
...
const FLOOR = { shared: 152, app: 8, appWhy: 'כניסה — סשן · נעילה · הרשאות · כתיבת משתמש · שכבת המראה' };
/* ⚠️ סוף פר-אפליקציה */
