...
const APP = {
  ...
  conflictKeys: { id: 'מפתח הטבלה', key: 'הגדרה לפי מפתח', month: 'יעד חודשי יחיד' },
   ...
   *  ⚠️ ⛔ משווה אחד בשם — ⚠️ `state.donors` הוא הרשימה היחידה שסדרה מוצהר: ⭐ השם נבדל בין הארבע
  ...
  actMap: 'ACTIONS',
  gapRows: [38, 68, 103, 105, 109, 127, 128, 132, 135, 143],
  ...
  dualRoleAllow: {
    '--bg': 'באנר העדכון הוא שבב הפוך — ⚠️ המשטח והדיו מתחלפים בתפקידם יחד, ⭐ והצמד נמדד עובר בשני המצבים',
    '--ink': 'באנר העדכון הוא שבב הפוך — ⚠️ המשטח והדיו מתחלפים בתפקידם יחד, ⭐ והצמד נמדד עובר בשני המצבים',
  },
  contrastAllow: {
    ...
    '.badge.ok|בהיר': 'תווית «בוצע» — ⚠️ גוון קיים ⛔ ולא תוצאה של הפיצול',
  ...
  tableProbe: {
    ...
    const _r66 = (() => {
      ...
      return _r66 && (callArity(c.code, '_ysRowsPaged').every((a) => a === 3)
          && /_ysRowsPaged\(function \(\) \{ return sb\.from\(m\.t\)/.test(c.code));
    },
     ...
     *  ניתנת למדידה במלואה. */
    147: (c) => c.hasCode(/G_PASS_ITER\s*=\s*100000\b/)
