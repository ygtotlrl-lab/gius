...
const APP = {
   ...
   *  רשומה ⛔ ולא לסחיפה. */
  appTokens: {
    '--bad': 'דיו הכישלון בשורת התנועה — מוצר ולא תשתית, ואינו בארבעתן',
    '--bad-soft': 'המילוי שמתחת לדיו הכישלון — משטח ולא דיו, ומוצר ולא תשתית',
    '--border-2': 'קו גבול משני לטבלאות התנועות — דרגה שנייה למושג הגבול, ומוצר',
    '--brand-fill': 'המילוי של גוון הזהות בכפתורים — מוצר ולא תשתית, ואינו בארבעתן',
    '--brand-ink': 'דיו גוון הזהות — תפקיד שני לאותו גוון, ולכן אסימון נפרד',
    '--brand-soft': 'שטיפת גוון הזהות מאחורי הכרטיס — מוצר ולא תשתית',
    '--gold': 'דיו ההתחייבות שנגבתה במלואה — מוצר ולא תשתית, ואינו בארבעתן',
    ...
    '--ok-soft': 'המילוי שמתחת לדיו ההצלחה — משטח ולא דיו, ומוצר ולא תשתית',
    '--radius-sm': 'רדיוס התגית הקטנה — נגזר מסולם הרדיוס, והשם קיים כאן בלבד',
    '--shadow-lg': 'הצל העמוק של המודאל — דרגה שנייה למושג הצל, ומוצר ולא תשתית',
    '--tab-h': 'גובה סרגל הלשוניות — מידה של המסך הזה, ואינה בשלוש האחרות',
    '--warn': 'דיו האזהרה בשורת המשימה — מוצר ולא תשתית, ואינו בארבעתן',
    '--warn-soft': 'המילוי שמתחת לדיו האזהרה — משטח ולא דיו, ומוצר ולא תשתית',
  },
  ...
  gapRows: [41, 73, 117, 119, 123, 145, 154, 59, 132],
  ...
  dualRoleAllow: {
    ...
    '--ok': 'נקודת השלב בלוח המשימות היא מילוי בלי דיו עליו — ⚠️ אין כלל שמצרף אותה לטקסט, ⭐ ולכן שני התפקידים אינם נפגשים ביחס',
    '--bad': 'נקודת השלב בלוח המשימות היא מילוי בלי דיו עליו — ⚠️ אין כלל שמצרף אותה לטקסט, ⭐ ולכן שני התפקידים אינם נפגשים ביחס',
  },
  contrastAllow: {
  ...
  tableProbe: {
     ...
     *  ניתנת למדידה במלואה. */
    165: (c) => c.hasCode(/G_PASS_ITER\s*=\s*100000\b/)
...
const FLOOR = { shared: 83, app: 10, appWhy: 'כניסה — סשן · נעילה · הרשאות · כתיבת משתמש · שכבת המראה; ומעליהם שני בלוקי גרף העמודות' };
/* ⚠️ סוף פר-אפליקציה */
