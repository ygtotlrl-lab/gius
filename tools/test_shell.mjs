...
if (!process.env.RD67_MUT) {
  ...
  console.log('\n— מוטציות (סבב 67) —');
  _mut('⛔ שינוי בית בליבת ShellActivity מזיז את החתימה ומפיל', 'android/app/src/main/java/com/gius/app/ShellActivity.java',
       (s) => s.replace(/\bonCreate\b/, 'onCreateRenamed'), true);
  _mut('⭐ מוטציית-נגד: הוספת שורה ריקה בסוף הקובץ ⛔ אינה מפילה', 'android/app/src/main/java/com/gius/app/ShellActivity.java',
       (s) => s.replace(/\s*$/, '\n'), false);
}
