...
const APP = {
  ...
  evictCalc: {
    seeds: ['collectedForPledge', 'viewHome', 'viewDonorCard', 'mirrorLoad', 'applyMirrorToState', 'sessSet', 'state'],
    /*  ⛔ הקוראים — ⚠️ פונקציה שנוקבת באחד מהם ומסכמת נמדדת, ⭐ או נכשלת. */
    readers: ['state.txns', 'state.tasks', 'state.pledges'],
    seed: `(function () {
  ...
  var txns = [];
  for (var d = 780; d >= 0; d -= 30) txns.push(row('t' + d, d, { donor_client_id: d % 60 ? 'd1' : 'd2', pledge_client_id: d % 60 ? 'p1' : 'p2', amount: 100 + d, txn_date: iso(now - d * DAY), category: 'כ', agent: 'א' }));
  localStorage.setItem('g_mirror_donors', JSON.stringify([row('d1', 900, { name: 'א' }), row('d2', 900, { name: 'ב' })]));
  localStorage.setItem('g_mirror_pledges', JSON.stringify([row('p1', 800, { donor_client_id: 'd1', amount: 5000, agent: 'א', due_date: iso(now + 30 * DAY) }), row('p2', 100, { donor_client_id: 'd2', amount: 900, agent: 'א' })]));
  localStorage.setItem('g_mirror_txns', JSON.stringify(txns));
  localStorage.setItem('g_mirror_tasks', JSON.stringify([row('k1', 700, { title: 'ישנה', assignee: 'א', stage: 'פתוח', due_date: iso(now - 690 * DAY) }), row('k2', 600, { title: 'הושלמה', assignee: 'א', stage: 'השלמה' }), row('k3', 5, { title: 'חדשה', assignee: 'א', stage: 'פתוח' })]));
})();`,
    load: "mirrorLoad(); applyMirrorToState(); sessSet({ full_name: 'א', role: 'admin' }); state.donorId = 'd1';",
    calls: {
      collectedForPledge: "['p1', 'p2'].map(collectedForPledge)",
      viewHome: 'viewHome()',
      viewDonorCard: 'viewDonorCard()',
    },
  },
  ...
  mutations: [
    ['  oldRecords: [],', "  oldRecords: [{ key: mirrorKey('g_txns'), ts: rowTs, syncedThrough: function () { return 0; } }],",
