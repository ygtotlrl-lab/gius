#!/usr/bin/env node
/*  test_offline_login.mjs — כניסה אופליין מרובת-משתמשים.
 *
 *  **מה נאכף:** ארבע טענות — (1) כניסה אופליין למשתמש שאינו האחרון שנכנס
 *  במכשיר; (2) סיסמה שגויה נדחית; (3) ⛔ הסיסמה אינה מופיעה באף מפתח
 *  אחסון, **בשום נתיב כתיבה**; (4) משתמש בלי טביעה מקבל את ההודעה הנכונה
 *  ⛔ ולא «סיסמה שגויה».
 *
 *  **הנימוק המדוד:** ⛔ סינון שיושב בנתיב אחד בלבד הוא בטוח **במקרה** ולא
 *  **במבנה** — ⚠️ ונמדד שנתיב שני עקף אותו.
 *
 *  **מה יישבר בלעדיו:** ⛔ סיסמה גלויה באחסון המקומי, ⚠️ באותו origin שבו
 *  חיות עוד שלוש אפליקציות; ⛔ והודעה מאוחדת שולחת את מי שחסרה לו טביעה
 *  לנסות את אותה סיסמה שוב.
 *
 ...
 *  ⚠️ מודל הכניסה עצמו נבדל: כתיבת משתמש כאן דורשת רשת ועוברת בגזירת
 *  הטביעה, ⛔ ולכן משתמש בלי טביעה אינו נוצר כלל ⛔ ואין מסלול השלמה
 *  שאפשר לבדוק. ⚠️ ובאפליקציה הרביעית אין קובץ כזה כלל.
 */
 ...
 *  טענה משותפת שאבדה. */
const FLOOR = { shared: 0, app: 77, appWhy: 'הכניסה האופליין — קיימת בשלוש, ומספר המסלולים נגזר ממסך ניהול המשתמשים שיש או שאין' };
...
function makeCtx(opts = {}) {
  ...
  const ctx = {
    ...
    /*  ⭐ המשתמש המחובר חי בזיכרון — הרתמה מדמה את מאפיין הגישה
...
/* ══════════════════════════════════════════════════════════════════════ */
console.log('\n▶ ש. שכבת המראה — מפתח נגזר והגירה חד-פעמית');
{
  const h = makeCtx();
...
console.log('\n▶ ב. כישלון גזירה **מאפס** את שני השדות');
{
  const h = makeCtx({ noCrypto: true });
...
console.log('\n▶ ג. ⭐ כניסה אופליין למשתמש שאינו האחרון שנכנס במכשיר');
{
  ...
  // ⭐ אין סשן על המכשיר; B נכנס מול הטביעה שבמראה בלבד.
  ...
  ok('⛔ ואין מפתח סשן על הדיסק', !('gius.session' in h.store));
...
console.log('\n▶ ו. ⛔ `password` אינו מגיע לאף מפתח localStorage');
{
  ...
  // 1) כתיבה מקומית של משתמש עם סיסמה — הנתיב שהדליף בעבר.
...
console.log('\n▶ ח. אינווריאנטות במקור עצמו');
{
  ...
  ok('הכניסה המקוונת לא קיבלה בדיקת פורמט סיסמה',
  ...
  /*  ⚠️ **שתי הטענות האלה התהפכו, במכוון.** עד אז הן אכפו
   *  ש-`gBackfillPassFp` מותנית ב-owner וברשת; הן אוכפות
  ...
  ok('⛔ gBackfillPassFp הוסרה', !/function\s+gBackfillPassFp/.test(SRC));
  ...
  // ⚠️ **תבנית ולא מספר קבוע**. הטענה המקורית קיבעה
  // `gius-v11`, ולכן היא הייתה **נכשלת על כל קידום עתידי** — כלומר
  // חוסמת בדיוק את מה שנדרש. זו אותה תקלה שכבר נמדדה כאן
  ...
  ok('המיגרציה אדיטיבית ואידמפוטנטית',
    /add column if not exists pass_salt text;[\s\S]*add column if not exists pass_fp\s+text;/
      .test(fs.readFileSync(path.join(ROOT, 'migrations/002_pass_fp.sql'), 'utf8')));
  ok('⛔ המיגרציה אינה נוגעת ב-password',
    !/\b(alter|drop|update)\b[^\n]*\bpassword\b/i.test(
      fs.readFileSync(path.join(ROOT, 'migrations/002_pass_fp.sql'), 'utf8')
