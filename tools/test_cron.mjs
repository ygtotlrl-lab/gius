...
/* ⭐ gius חולקת את הפרויקט `zrftjkghhjhqzopvdzou` עם kupa, ⛔ והמיגרציה של
   רשימת-ההיתר יושבת כאן — ⚠️ ולכן `sisterKeys` נושאת את מפתחות `k_*`
   שהאחות כותבת לאותה `sh_backup`. */
/* ⚠️ סוף פר-אפליקציה */
const APP = {
  /*  ⭐ מפתחות הגיבוי שהרשימה במסד חייבת להכיל — ⛔ **אינו נגזר**: שמות טבלאות שהגיבוי כותב, ⚠️ והמסד הוא מה שנמדד מולם */
  keys: ['g_donors', 'g_pledges', 'g_txns', 'g_tasks', 'g_targets', 'g_settings', 'g_users'],
...
const FLOOR = { shared: 25, app: 32, appWhy: 'מספר הטבלאות והמשימות — הבעלות על מיגרציית הגיבוי כאן' };
/* ⚠️ סוף פר-אפליקציה */
