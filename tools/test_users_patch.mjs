#!/usr/bin/env node
/*  בדיקת סבב 26 — נתיב עדכון חלקי למראת המשתמשים.
 *
 *  ⚠️ **הבדיקה רצה על הקוד האמיתי המחולץ מ-`index.html`**, לא על העתק.
 *  הפונקציות נחתכות מהקובץ לפי שם (התאמת סוגריים), מורצות ב-`vm` מעל
 *  רתמה מינימלית, ונבדקות מולה. מוטציה בקוד האמיתי מפילה כאן טענה.
 *  ⚠️ **פרטי ל-gius** — בניגוד ל-`check-status-area.mjs` ול-`check-docs.mjs`
 *  הוא אינו זהה לאף ריפו אחר, ואין ליישר אותו.
 *
 *  שלוש האינווריאנטות שהסבב נדרש להן:
 *    1. ⛔ עדכון חלקי אינו מכניס `password` — לא ל-`MIRROR` ולא לדיסק.
 *       זו הנקודה שנשברה ב-hanhala בסבב 21 וכאן בסבב 23: מסנן שיושב
 *       בנתיב אחד בלבד, ונתיב שני שעוקף אותו.
