...
const APP = {
  ...
  accept: { expose: [], setup: '', cases: [], why: '⛔ אין כאן חשבון שחי באפליקציה אחת — ⚠️ הסכומים נגזרים מהתנועות בזמן קריאה' },
  /*  ⛔ ידיות הגרירה — ⚠️ **מה נכנס**: כל בורר שנושא `touch-action:none`
   *  ⟵ למה הוא ידית; ⛔ **ומה מפיל**: בורר שאינו כאן, ⭐ והכרזה שאין
   *  לה כלל. */
  dragHandles: [
    { sel: '.kcard > .grip', why: 'ידית כרטיס הקנבן — ממנה הכרטיס עובר בין העמודות' },
  ],
   ...
   *  ⛔ והגרירה עצמה אחת. */
  drag: {
    expose: ['state', 'render', 'STAGES'],
    setup: "(function () { var S = window.__acc.state;" +
      /*  ⛔ המשתמש נקבע בזיכרון לפני ההזרעה — ⚠️ `render()` מציירת מסך
       *  כניסה כל עוד אין משתמש, ⭐ ובהתקנה טרייה אין במסד משתמש:
       *  ⛔ והלוח לא היה מגיע למסך כלל. */
      " if (!S.user) S.user = { name: '\u05d1\u05d3\u05d9\u05e7\u05d4', role: 'admin' };" +
      " if (!S.tasks.length) S.tasks.push({ client_id: 'zz1', id: 'zz1'," +
      "   title: '\u05d1\u05d3\u05d9\u05e7\u05d4', stage: window.__acc.STAGES[0], deleted: false, updated_at: 1 });" +
      " S.screen = 'tasks'; window.__acc.render();" +
      " return document.querySelectorAll('.kcard').length === 1; })()",
    grip: '.kcard > .grip',
    target: '.kcol:nth-of-type(2)',
    order: "(function () { var c = document.querySelector('.kcard');" +
      " return c && c.closest('.kcol') ? c.closest('.kcol').dataset.stage : ''; })()",
    mut: ["if (col && col.dataset.stage) moveTask(el.dataset.id, col.dataset.stage);",
          "if (false) moveTask(el.dataset.id, col.dataset.stage);"],
  },
  dragWhy: '',
};
/* ── סוף APP ───────────────────────────────────────────────────────────── */
